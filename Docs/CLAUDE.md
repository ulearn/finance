...
- Subfolders inside scripts shoud be as short as possible to describe the grouping of functions that folder is attempt to achieve eg fins/scripts/xero/recon/ is 
    for Xero Reconciliation analysis (we don't need to call xero/reconciliation-analysis/...)    
- Temporary code for parsing and analysis that is one-off (or one session) in nature and will be discarded afterwards can be filed/stored in .claude/tmp/
- Code that is part of an ongoing structure and usaage will be put into scripts/../...
- We usually try avoid hardcoding long data structures into the business logic files - use an external json file to store and call that in the bus logic file
